...
def add_data(model: Model, data_path: str):
    ...
    for f in files:
        ...
        model.get_predicate(pred).add_data_file(partition_map[ptype],os.path.join(data_path, f))
...
def get_predicates(model: Model, psl_pred: str):
    with open(psl_pred, "r") as f:
        ...
        for pred in f:
            ...
            predicate = Predicate(p[0], closed = True if p[2] == "closed" else False, size=int(p[1]))
